...
- 症状: Claude Code内の `/mcp` を開いても `codex-reviewer` が一覧に出ない、または `connected` にならない。
...
- 確認: `agents/` 配下に4ファイルが正しく配置されているか、frontmatterの `name:` フィールドが一致しているか。Claude Codeのバージョンによっては `/agents` の一覧ウィザードが使えない場合があるが、それ自体はAgentツールによる直接起動を妨げない。
- 継承元: `subagents.md` の「手動確認手順」

## Skillが表示されない・起動しない

- 症状: `/multi-agent-workflow:multi-agent-design` を入力しても認識されない。
- 確認: `skills/multi-agent-design/SKILL.md` の配置パス（ディレクトリ名とfrontmatterの `name:` が一致しているか）、Claude Codeの再起動有無。
- 継承元: `multi-agent-design-skill.md` の「Skillの場所とSkill名」

## Windows Git Bashで `codex` / `claude` コマンドが見つからない

- 症状: Windows Git Bash上で `command -v codex` や `command -v claude` が解決しない、または `codex mcp-server` の起動に失敗する。
...
- 対応: 自動マージは行わない。既存ファイルの内容を人が確認し、必要なサーバー設定（`codex-reviewer` のエントリ）だけを手動で追記するかどうかを判断する。既存の他サーバー設定を消さない。
- 新規ガイダンスであり、元文書からの直接継承ではない
