...
- 配置: [`../../skills/multi-agent-design/SKILL.md`](../../skills/multi-agent-design/SKILL.md)（プラグインスコープ）

## 実行方法

- `/multi-agent-workflow:multi-agent-design [設計タスク]` の形で、ユーザーが明示的に起動する
...
このSkill自身はコードを実行するプログラムではなく、親のClaude Codeセッションが読み、その指示に従って各フェーズを順番に実行する手順書である。Claude project subagents（`agents/`）とCodex MCP（`codex-reviewer`）はいずれも親セッションから直接呼び出し、subagent同士やCodexから別ロールを連鎖起動する構成にはしていない（[`subagents.md`](subagents.md)の「subagent同士が直接連鎖しないこと」を継承）。
...
1. Terminal版Claude Codeを、`skills/multi-agent-design/SKILL.md` を含むリポジトリ/worktreeのルートから起動する
...
4. `/multi-agent-workflow:multi-agent-design [設計タスク]` を明示的に起動する
