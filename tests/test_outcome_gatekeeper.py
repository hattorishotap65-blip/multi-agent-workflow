...
POKEMON_PROFILE = ROOT / "examples" / "app-profiles" / "pokemon-ai.example.json"
RAG_PROFILE = ROOT / "examples" / "app-profiles" / "rag-quality.example.json"
...
class SampleProfileTests(unittest.TestCase):
    def test_pokemon_example_validates(self):
...
class SafetyAndTemplateTests(unittest.TestCase):
    ...
    def test_claude_skill_frontmatter_and_entrypoints(self):
        skill = (ROOT / "skills" / "outcome-improvement-cycle" / "SKILL.md").read_text(
            encoding="utf-8"
        )
